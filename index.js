...
function generateDownloadTable() {
    ...
    lines.push(
        "|" + "|".repeat(cols.length + 1).replace(/\|/g, "------") + "|",
    );
...
function getCommits() {
    let prefix, stdout;
    if (!PREVIOUS_TAG) {
        stdout = execSync('git log --pretty=format:"- %s (%h)" --no-merges', {
            encoding: "utf-8",
        });
        prefix = `生成从初始提交到 ${VERSION} 的提交记录\n`;
    } else {
        stdout = execSync(
            `git log ${PREVIOUS_TAG}..${VERSION} --pretty=format:"- %s (%h)" --no-merges`,
            { encoding: "utf-8" },
        );
...
function main() {
    ...
    if (INCLUDE_DOWNLOAD_TABLE) {
        ...
        if (PLATFORMS.includes("windows")) {
            notes.push(
                `> 💡 **提示**：Windows 用户下载 \`.exe\` 文件，其他系统下载后需要添加执行权限 \`chmod +x ${BINARY_NAME}-*\``,
            );
            notes.push("");
        }
        notes.push("---\n");
    ...
    if (INCLUDE_INSTALL_GUIDE && INCLUDE_DOWNLOAD_TABLE) {
        ...
        notes.push(
            `2. Linux/macOS/BSD 用户需要赋予执行权限：\`chmod +x ${BINARY_NAME}-*\`（建议直接将文件改名为 ${BINARY_NAME} 方便调用和后续的升级）`,
        );
        notes.push(
            `3. 运行程序（建议直接将文件改名为 ${BINARY_NAME} 方便调用）\`${BINARY_NAME} help\``,
        );
    }
